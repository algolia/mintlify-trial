...
```mermaid
flowchart TD
ChooseStrategy{"Strategy"} --> AF["Algolia first"] & RF["RMP first"] & P["Parallel"]
AF --> AF_Search["User searches for "running shoes""]
RF --> RF_Search["User searches for "running shoes""]
P --> P_Search["User searches for "running shoes""]
AF_Search --> AF1["Query Algolia for regular results"]
AF1 --> AF2["Query RMP for sponsored results"]
AF2 --> Merge["Merge and deduplicate results"]
RF_Search --> RF1["Query RMP for sponsored results"]
RF1 --> RF2["Query Algolia for regular results"]
RF2 --> Merge
P_Search --> P1["Query Algolia and RMP in parallel"]
P1 --> P2["Wait for both responses"]
P2 --> Merge
Merge --> Display["Display final product list"]
```
...
<Frame>
  ![](https://files.readme.io/6ee6bd9-image.png)
</Frame>