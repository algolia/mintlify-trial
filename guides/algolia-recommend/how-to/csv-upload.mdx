...
1. Select a model you want to train and click **Start using**.
1. In the **Select data source** section, select an Algolia index as a source for your recommendations.
1. In the **Collect events** section, click **Upload .csv** to upload your CSV file with the historical events.

    ![Import a CSV file with historical events for training your Recommend model](/images/recommend/csv-upload.png)

Once you collected enough [click and conversion events](/guides/send-events/get-started/),
Algolia Recommend only uses those events for training the model and discards the events from the CSV file.
