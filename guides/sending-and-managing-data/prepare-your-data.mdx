...
<Info>
  - [Data formats for relevant search](https://academy.algolia.com/guides/5d902fea-c439-11ee-96f2-06d3877cd55d/tasks/861fc984-b0b1-11ee-a537-0215f0d95509)
</Info>
...
Each record should have enough information for users to find it quickly. When users search for something, Algolia returns records as results that match their search.
...
<Info>
  - [Reducing record size](/guides/sending-and-managing-data/prepare-your-data/how-to/reducing-object-size/)
  ...
  - [Structure ecommerce product records](/guides/sending-and-managing-data/prepare-your-data/how-to/ecommerce-records/)
</Info>
...
For example, you could use `director`, `cast`, `country`, `content_rating`, and `genres` and display them as refinement lists in your search experience, _and_ `release_year` to display a range slider. Declare them with the [`attributesForFaceting`](/api-reference/api-parameters/attributesForFaceting/) parameter.
...
Without anything else to break the tie, Algolia falls back on the `objectID` in alphanumeric order, which isn't helpful. A better way to break ties is to compare _meaningful_ information. For movies, you could use the `review_scores` attribute. However, in the preceding example, you have several scores, you may want to compute them into a global one and use them in custom ranking. The computed attribute would look like this:
...
<Info>
  ...
  - [Controlling precision of custom ranking metrics](/guides/managing-results/must-do/custom-ranking/how-to/controlling-custom-ranking-metrics-precision/)
