...
import UpdateHTML from '/snippets/insights/update-html.mdx';

<UpdateHTML />

To track filter events, add a `data-insights-filter` attribute to each filter element (format: `${attribute}:${value}`):

import UpdateFilters from '/snippets/insights/update-html-filters.mdx';
...
import HitsViewed from '/snippets/insights/hits-viewed.mdx';
