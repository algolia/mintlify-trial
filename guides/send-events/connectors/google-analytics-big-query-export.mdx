...
<Card title="Beta" icon="vial" href="/content/components/card-group">
...
1. In the [BigQuery console](https://console.cloud.google.com/bigquery),
   select the dataset with the BigQuery Export, open the **Actions** menu and select **Share > Manage Permissions.**

1. Click **Add Principal**. In the **New principals** text box, enter the name of the service account.

1. In the **Select a role** menu, select the **BigQuery > BigQuery Data Viewer** role.
...
1. Select your service account. Under **Actions**,  select **Manage keys**.

1. Select **Add key > Create new key**.

1. In the **Key type** menu, select **JSON**.
...
1. On the left sidebar, select <Icon icon="magnifying-glass" color="5468ff" iconType="regular" /> **Search**.
1. Select your Algolia index:

    ![Select your Algolia application and index](/images/select-index.png)
    
1. Go to the Algolia dashboard and select your Algolia application.
1. On the left sidebar, select <Icon icon="database" iconType="regular" /> **Data sources**.
1. On the [**Connectors**](https://dashboard.algolia.com/connectors/) page, select the [**Google Analytics 4 BigQuery Export**](https://dashboard.algolia.com/connectors/google-analytics-4-bq-export/create) and click **Connect**.
...
1. In the **Google Service Account** box,
   select **Create a new Google Service Account authentication**.

1. In the **Create a new Google Service Account authentication** menu,
...
1. In the **Enter your BigQuery table details** section,
...
1. Enter an index name for Algolia events imported by this connection.
1. To create Algolia credentials for the connector, click **Create one for me**.
1. Click **Save and continue**.
